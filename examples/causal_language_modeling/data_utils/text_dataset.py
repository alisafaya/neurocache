...
flags.DEFINE_string("default_data_dir", None, "Default directory where data is stored.")
FLAGS = flags.FLAGS
...
def load_text_dataset(
    ...
    logging.info(
        "Loading text data set %s, split=%s, shape=(%d, %d), vocabsize=%d",
        name,
        split,
        batch_size,
        sequence_length,
    ...
    if shard_dataset:
        logging.info("Shards: %d of %d", process_index, process_count)
        shard_info = seqio.ShardInfo(index=process_index, num_shards=process_count)
    else:
        shard_info = None
    ...
    def extract_fn(article):
        if "mask" in article:
            return article["targets"], article["mask"]
        else:
...
def pretty_print_article(
    article, vocab_map: Mapping[str, Optional[seqio.Vocabulary]], max_length: int = 60
) -> str:
...
def decode_tokens(tokens: Any, vocab: seqio.Vocabulary, max_length: int) -> str:
    ...
    if np.ndim(tokens) == 1:
        ...
        jtstr = ",\n    ".join([decode_tokens_1d(s, vocab, max_length) for s in tokens])
        tstr += f"[\n    {jtstr}\n  ]"
    else:
...
def decode_tokens_1d(
    tokens: Any, vocab: Any, max_length: int, raw_string: bool = False
) -> Union[str, bytes]:
    ...
    if isinstance(tstr, str):
        ...
        tstr = bytes(str(tstr).encode("utf-8"))
...
def split_article(
    ...
    assert np.ndim(tokens) == 1
    if include_loss_mask and loss_mask is None:
        loss_mask = loss_mask_from_tokens(tokens, split)

    if loss_mask is not None:
        assert np.ndim(loss_mask) == 1
        assert len(loss_mask) == len(tokens)
        loss_mask = loss_mask.astype(np.int32)
    ...
    for k in range(0, len(tokens), sequence_length - 1):
        ...
        if include_loss_mask:
            segment_loss_mask = pad_chunk(
                loss_mask[k : k + sequence_length], sequence_length
            ).astype(np.int32)
        else:
            segment_loss_mask = np.array(True, dtype=np.int32)  # dummy mask
        yield (segment, segment_loss_mask)


def nonzero_tokens(tokens: np.ndarray, loss_mask: Optional[np.ndarray]) -> Sequence[int]:
...
def loss_mask_from_tokens(tokens: np.ndarray, split: str) -> np.ndarray:
    ...
    if start_indices:
        ...
        if len(start_indices) > 1 or len(end_indices) > 1:
            logging.error(
                "Multiple start or end tokens for loss mask: %s, %s", start_indices, end_indices
            )
...
def _batched_interleave_generator(
    ...
    while any(still_reading):
        ...
        for i in range(0, batch_size):
            ...
            if include_loss_mask:
                loss_mask[i] = post_map_func(loss_mask_i).astype(np.int32)  # handles None
        ...
        #     bchars = decode_tokens_1d(toks,
        #                               vocab,
        #                               max_length=len(targets[i]),
        #                               raw_string=True)
...
def split_and_batch(
    ...
    def wrap_split_article(article):
        ...
        if isinstance(tokens, str) or isinstance(tokens, bytes):
            ...
            lossmask = tokens[1].astype(np.int32)
            tokenids = tokens[0].astype(np.int32)
        ...
        return split_article(
            tokens, sequence_length, split=split, include_loss_mask=include_loss_mask
        )
    ...
    if include_loss_mask:
        out_sig["loss_mask"] = tf.TensorSpec(shape=(batch_size, sequence_length), dtype=tf.int32)

    cds = tf.data.Dataset.from_generator(
        wrap_batched_interleave_generator, output_signature=out_sig
    )
    return cds
